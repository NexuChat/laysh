...
side-effect free, and returns exactly the declared finite outputs.

Shared pivotal-state contract:

- After `/* LAYSH_SHARED_MODEL: modelState */`, define one pure named state-object function.
- Render and `test(inputs)` call the same model function: `const state = modelState(value);`; use the
  real `state.output` for pivotal visuals, with no duplicate formula or no-op call.

Shared scene-geometry contract:

- After each fit/clamp, set `canvas.__layshSceneGeometry` to nonempty closed v1.0 samples:
  `phase: "post_fit"`, viewport, state, scientific geometry, and relations.
- Do not report decorative particles, glows, text, chips, trails, or texture. Each object sets
  `clippingPolicy`. Declare one relation for every pair in `objects` with `overlapPolicy`,
  `contactPolicy`, and `minimumClearance`; use `scientific_occlusion` only for physical intent.
- Shape: `[{ schemaVersion: "1.0", phase: "post_fit", viewport: { width, height, safeInset: 0 },`
...
Use only canvas/context, Math, Number, arrays, and plain objects. No document, network,
storage, navigation, dynamic code, workers, timers, sensors, audio, clipboard, console, URLs, or
`requestAnimationFrame`. Keep source ≤96 KiB in UTF-8 bytes. The fixed physics/spec is immutable.

Visual contract:

- Create layered scene depth: at least three visible depth layers—gradient, near/far bodies, and
  restrained texture; never flat.
- Make physical light beautiful and physically consistent; never draw light through an opaque body.
  Show glow, true occlusion, soft shadow, and a subtle shadow cone.
- Add subtle idle motion via private `visualPhase` on the shell's ~12 fps same-value redraw, only
  outside reduced motion; it is visual-only, never physics or `test(inputs)`.
- Add smooth reactive feedback to parameter changes; preserve the prior display value and change
  more than text.
...
- If schematic and observer views share a canvas, label them `منظر علوي` / `كما يبدو من الأرض` or
  `Top view` / `View from Earth`. Keep labels inside the canvas.
